...
const CARDS = [
  ...
  { href: '/admin/products',   icon: '🛒', label: 'Manage Products',       desc: 'Add, edit, delete products',   color: 'hover:border-yellow-500/50' },
  { href: '/admin/banners',    icon: '🖼',  label: 'Homepage Banners',      desc: 'Edit hero banners',            color: 'hover:border-purple-500/50' },
  ...
  { href: '/admin/delivery',   icon: '🗺',  label: 'Delivery Zones',        desc: 'Zone fees & settings',         color: 'hover:border-cyan-500/50'   },
  { href: '/admin/drivers',    icon: '🚚', label: 'Drivers & Vehicles',    desc: 'Manage dispatch team',         color: 'hover:border-orange-500/50' },
  { href: '/admin/settings',   icon: '⚙',  label: 'Site Settings & Logo',  desc: 'Colors, logo, social links',   color: 'hover:border-gray-500/50'   },
...
export default function AdminPage() {
  ...
  const stats = [
    { label: 'Total Products', value: mounted ? products.length : '-',                             icon: '🛒', color: 'text-blue-400'   },
    { label: 'On Homepage',    value: mounted ? products.filter(p => p.show_on_home).length : '-', icon: '🏠', color: 'text-yellow-400' },
    { label: 'Active Banners', value: mounted ? banners.filter(b => b.is_active).length : '-',    icon: '🖼',  color: 'text-purple-400' },
    { label: 'Low Stock',      value: mounted ? products.filter(p => p.stock < 10).length : '-',  icon: '⚠',  color: 'text-red-400'    },
  ]
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-10">
        {CARDS.map(card => (
          <Link
            key={card.href}
            href={card.href}
            className={`bg-gray-900 border border-gray-800 rounded-xl p-4 transition-all duration-150 group cursor-pointer hover:bg-gray-800/80 ${card.color}`}
          >
              ...
              <tbody>
                {products.slice(0, 6).map(p => (
                  <tr key={p.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${p.show_on_home ? 'bg-yellow-500/20 text-yellow-400' : 'bg-gray-800 text-gray-500'}`}>
